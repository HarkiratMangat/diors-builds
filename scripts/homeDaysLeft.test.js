#!/usr/bin/env node
// scripts/homeDaysLeft.test.js — Home's restored fourth masthead figure ("days left", build-out plan
// 2026-09-06 01:33 EDT) and the two clarify/onboard copy changes beside it.
//
// Same convention as scripts/portalAnalytics.test.js: portal/ui/home.js cannot be `require`d or
// `import`ed directly in bare Node (it pulls in the buildless vendor bundle, which only exists after
// `npm run build`), so the pure arithmetic (dayOf/dday/seasonDaysLeft) is extracted from the REAL
// source text by name and evaluated with `new Function`, with only the one true external dependency
// — season.js's own `seasonLastDeadline` — supplied as a stub. Everything else under test is the
// actual bytes on disk, not a hand-copied re-implementation of them.
//
...
// Slices one function's source out by name, walking braces rather than regexing them so a one-line
// arrow (dayOf/dday, no braces at all) and a multi-line `function` declaration (seasonDaysLeft) both
// come out whole and nothing else.

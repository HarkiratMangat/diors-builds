...
function composerReason(state, type) {
    ...
    // 🔴 A DRAW'S SECOND DATE IS OPTIONAL AND IS NOT THE DRAW'S OWN. `newDraws[].date` is the only date the draw record has (models/SeasonalData.js), so a closing date here does not go on the draw at all — it stages a SECOND op, a calendar row with category 'Draw', which is what a draw window has always been. Blank is therefore a complete answer and must not read as an unfinished one; only a typed value that does not resolve, or one that closes before the release, can hold the button.
    if (type.windowable) {
...
// The patch-note image slots, exactly as /manage splits them: the first five URLs are slot 1 and the next five slot 2 (commands/manage.js's buildPatchAddSeasonModal collects two paragraph fields for the same reason). One textarea rather than two, because a reader pasting ten links should not have to count to five — the split is arithmetic, and arithmetic belongs on this side of the form.
...
function composerFields(state, type) {
    ...
    // An event's Double CP mark is a real stored field (`calendar[].isDoubleCP`), not a label — /calendar renders it, so it has to reach the payload rather than the title.
    if (type.shape === 'span') return { title, startDate: state.aIso, endDate: state.bIso, isDoubleCP: !!state.doubleCP };
    // A patch note is a point whose OTHER fields are the whole of it. buildSeasonAddOp used to hardcode an empty description and two empty URL slots, so the one control on this page that could create a publication created an empty one — /manage was the only place the content could be written.
    ...
    if (type.windowable) {
        // ⚠️ THE NOTE IS AN ITEM, NOT A FIELD, and that is the bot's own shape rather than a shortcut: utils/adminParser.js's parseItemLine returns exactly {tier:'comment', name} for a "-# …" line and core/ops/draws.js stores it in the same `items` array. Giving the note its own payload key would invent a second place for a thing the record already has one place for.
        ...
        // The window is reported separately from the draw's own date: buildSeasonAddOps turns it into the second op, and a builder that returns one op must never see it as part of the first.
